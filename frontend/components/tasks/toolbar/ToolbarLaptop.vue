  ...
  >
    <v-row no-gutters>
      <v-btn-toggle>
        <!-- Undo Redo -->
        <button-undo-redo
          @click:undo="$emit('click:undo')"
          @shortkey:undo="$emit('shortkey:undo')"
          @click:redo="$emit('click:redo')"
          @shortkey:redo="$emit('shortkey:redo')"
        />
        <button-review
...
import ButtonUndoRedo from './buttons/ButtonUndoRedo.vue'
